...
logs_path = '/tmp/tensorflow_logs/example_cnn_mnist'

#下载并加载数据
mnist = input_data.read_data_sets("MNIST_data/", one_hot=True)  #下载并加载mnist数据
sess = tf.InteractiveSession()
x = tf.placeholder("float", shape=[None, 784],name='InputData')                  #输入的数据占位符
y_ = tf.placeholder("float", shape=[None, 10],name='LabelData')                  #输入的标签占位符
